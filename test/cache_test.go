...
var cachedir = fmt.Sprintf("~/%s", jsoncache.DefaultCacheDir)
...
func TestCache(t *testing.T) {
	...
	t.Run("GetCacheFilepath()", func(t *testing.T) {
		...
		got, sts := jsoncache.ExtractPath(fp, cachedir)
		if is.Error(sts) {
			t.Errorf("Failed to extract path: %s", sts.Message())
		}
		wanted := fmt.Sprintf("/%s.json", cachekey)
		if got != wanted {
	...
	t.Run("Get(does-not-exist)", func(t *testing.T) {
		_ = c.Clear(cachekey)
		data, sts := getCacheValue(t, c, &validateArgs{
			MustFail: true,
		})
		if is.Success(sts) {
	...
	t.Run("Set(),Get()", func(t *testing.T) {
		...
		b, sts = getCacheValue(t, c, &validateArgs{
			MustSucceed: true,
		})
		if is.Error(sts) {
...
func getCacheValue(t *testing.T, c *jsoncache.Cache, args *validateArgs) (data []byte, sts status.Status) {
	var ok bool
